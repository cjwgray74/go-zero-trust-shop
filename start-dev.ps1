...
<#
  start-dev.ps1 (v3)
  One-click dev runner for go-zero-trust-shop on Windows using Podman Machine + Terraform.

  What it does:
    1) Starts Podman Machine if needed
    2) Brings up Vault + Postgres via podman-compose (inside the machine)
    3) Waits for ports (Vault 8200, Postgres 55432)
    4) Runs Terraform (init + import dev mounts + apply) to configure Vault
    5) Reads VAULT_ROLE_ID / VAULT_SECRET_ID from Terraform outputs
    6) Exports env vars and runs auth-svc (go run .)
  ...
  Optional:
    -SkipRun  → do everything except starting the Go service
#>
...
<#
  ...
  What it does:
    ...
    2) Brings up Vault + Postgres via podman-compose (inside the machine)
    3) Waits for ports (Vault 8200, Postgres 55432)
    4) Runs Terraform (init + import dev mounts + apply) to configure Vault (AppRole + DB role + policy)
    5) Reads VAULT_ROLE_ID / VAULT_SECRET_ID from Terraform outputs
    6) Exports env vars and runs auth-svc (go run .)
...
$deploy = Join-Path $root "deploy"
$svc    = Join-Path $root "services\auth-svc"
$tf     = Join-Path $deploy "terraform"

# Use Podman Machine ROOT connection for compose + exec
$connection = "podman-machine-default-root"
$composeProvider = "podman-compose"  # pip install podman-compose
...
function Wait-Port($HostName, $PortNumber, $Label) {
  Write-Host ("[WAIT] " + $Label + " on " + $HostName + ":" + $PortNumber) -ForegroundColor DarkCyan
  $ok = $false
  for ($i=1; $i -le 180; $i++) { # up to ~90s
    $test = Test-NetConnection -ComputerName $HostName -Port $PortNumber -WarningAction SilentlyContinue
    if ($test.TcpTestSucceeded) { $ok = $true; break }
    Start-Sleep -Milliseconds 500
    if ($i % 10 -eq 0) { Write-Host "[WAIT] still waiting..." } 
  }
  if (-not $ok) { throw "Timeout waiting for " + $Label + " on " + $HostName + ":" + $PortNumber }
}

try {
  # --- 1) Start Podman Machine (if needed) ---
  Info "Ensuring Podman Machine is running..."
  try { podman machine start | Out-Null } catch {}

  # --- 2) Bring the stack up (compose inside machine) ---
  Info "Bringing stack up with podman compose (inside machine)..."
  Set-Location $deploy
  $env:PODMAN_COMPOSE_PROVIDER = $composeProvider

  # Create or ensure up
  podman --connection $connection compose up -d | Out-Null

  Write-Host ""
  Write-Host "Containers currently running:"
  podman --connection $connection ps
  ...
  # --- 4) Terraform: init + import dev mounts + apply ---
  if (-not (Test-Path $tf)) { throw "Terraform folder not found: " + $tf }
  Set-Location $tf

  # Vault dev env (root token)
  $env:VAULT_ADDR  = "http://127.0.0.1:8200"
  ...
  # Import common dev mounts if they already exist (idempotent)
  Info "Importing existing Vault mounts if present (safe to re-run)..."
  try { terraform import vault_auth_backend.approle approle    | Out-Null } catch {}
  try { terraform import vault_mount.database           database | Out-Null } catch {}

  Info "Applying Terraform (auto-approve)..."
  terraform apply -auto-approve | Write-Host

  # Read outputs
  $ROLE_ID   = terraform output -raw VAULT_ROLE_ID
  $SECRET_ID = terraform output -raw VAULT_SECRET_ID

  Ok "ROLE_ID retrieved"
  Ok "SECRET_ID retrieved (hidden)"

  # --- 5) If not skipping, export env & run the Go service ---
  if (-not $SkipRun) {
    Info "Starting auth-svc with fresh Role/Secret (Ctrl+C to stop) ..."
    ...
    Write-Host ("  VAULT_ADDR=" + $env:VAULT_ADDR)
    Write-Host ("  VAULT_ROLE_ID=" + $env:VAULT_ROLE_ID)
    Write-Host ("  VAULT_SECRET_ID=(hidden)")
    Write-Host ("  DB_HOST=" + $env:DB_HOST)
    Write-Host ("  DB_PORT=" + $env:DB_PORT)
    ...
    Warn "(You passed -SkipRun) Use these to run the service manually:"
    Write-Host ("  cd """ + $svc + """")
    Write-Host ("  `$env:PORT='8083'")
    Write-Host ("  `$env:VAULT_ADDR='http://127.0.0.1:8200'")
    Write-Host ("  `$env:VAULT_ROLE_ID='" + $ROLE_ID + "'")
